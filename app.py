...
# -------------------- SUBMISSIONS PER COUNTY --------------------
county_counts = filtered_df['County'].value_counts().reset_index()
county_counts.columns = ['County', 'Submissions']
st.subheader("📊 Submissions per County")
st.dataframe(county_counts)

csv_county_data = county_counts.to_csv(index=False).encode('utf-8')
st.download_button("📥 Download Submissions by County CSV", data=csv_county_data, file_name=f"Submissions_Per_County_{datetime.now().date()}.csv", mime='text/csv')
